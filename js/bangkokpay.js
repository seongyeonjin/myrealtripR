(()=>{
    ...
    let timer=null , interval=3000 , imgurl='',$himg=3 ;
    let cnt1=0 , cnt2=0, cnt=0;
    timer=setInterval(make,interval);
    function make() {
        cnt++;
        if(cnt > $himg ) cnt=0;
        imgurl='images/pack-bangkok/hotel'+cnt+'.jpg'
    ...
    $khj2.addEventListener('click',e=>{
      $popspan.textContent='[성인]방콕 자유여행(3인단독)'
      cnt2++;
      if(cnt2 > 9 ) {
         cnt2 = 9;
      }
      ...
      $pp.textContent=`${cnt2*465000}원`
      $st.textContent=`${cnt2*465000}원`
})
